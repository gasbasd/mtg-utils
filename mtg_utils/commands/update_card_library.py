    ...
    if decks_with_sharing:
        ...
        for deck_name, deck_config in decks_with_sharing:
            ...
            if len(shared_decks) > 1:
                ...
                common_shared_cards = {card: decks_list for card, decks_list in shared_cards_info.items() if len(decks_list) > 1}
                # Cards in only one shared deck (exclusive)
                exclusive_shared_cards = {card: decks_list for card, decks_list in shared_cards_info.items() if len(decks_list) == 1}
                ...
                if common_shared_cards:
                    ...
                    print(f"    (no cards common across all shared decks)")
                    ...
                    if shared_deck_name in deck_cards:
                        ...
                        for card_name in sorted(current_deck_cards.keys()):
                            if card_name in exclusive_shared_cards and card_name in shared_deck_cards:
                                ...
                                if len(shared_cards_info[card_name]) == 1 and shared_cards_info[card_name][0][0] == shared_deck_name:
...
def update_card_library(config_file) -> None:
    ...
    available_cards = _calculate_available_cards(owned_cards, decks, purchased_cards, config)
